...
fromVector' gen points
  ...
  where
    ...
    node =
        VpNode
            { vpInBranch = fromVector' gen (G.map (\(i, q, _) -> (i, q)) inb)
...
nearNeighbors (VpNode ib ob vp ix mu) radius q
    | isAllIn
        && not onlyIn =
        validPoint (nearNeighbors ob radius q ++ allIn)
...
getWithRadius (VpNode ib ob vp ix mu) r0 q
    | goBoth
        && d < mu =
        getSmallest2 (\r -> getWithRadius ib r q) (\r -> getWithRadius ob r q) std
